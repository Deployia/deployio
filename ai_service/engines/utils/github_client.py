...
class GitHubClient:
            ...
            if response.status == 200:
                ...
                for item in data.get("tree", []):
                    if (item.get("type") == "blob" and 
                        item.get("size", 0) < 100000 and  # Max 100KB
                        self._is_text_file(item.get("path", ""))):
                ...
                return sorted(filtered_files, key=self._file_importance_score, reverse=True)
            else:
    ...
    async def _get_key_files_content(
        ...
        important_files = [
            f for f in file_tree[:50]  # Limit to top 50 files
    ...
    def _is_text_file(self, file_path: str) -> bool:
        ...
        text_extensions = {
            '.js', '.jsx', '.ts', '.tsx', '.py', '.java', '.go', '.php',
            '.rb', '.rs', '.cpp', '.c', '.h', '.cs', '.kt', '.scala',
            '.json', '.yaml', '.yml', '.xml', '.toml', '.ini', '.cfg',
            '.md', '.txt', '.dockerfile', '.sql', '.sh', '.bat', '.ps1',
    ...
    def _is_key_file(self, file_path: str) -> bool:
        ...
        key_files = {
            ...
            'angular.json', 'ember-cli-build.js',
