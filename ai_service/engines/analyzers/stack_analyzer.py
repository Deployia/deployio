...
class StackAnalyzer:
    ...
    def _init_detection_patterns(self):
        ...
        self.framework_patterns = {
            "react": {
                ...
                "files": ["package.json", "public/index.html", "src/index.js", "src/App.js"],
                "indicators": ["src/App.js", "src/index.js", "public/favicon.ico"],
                "confidence_boost": 0.15
            },
            "vue": {
                "directories": ["src/", "public/", "components/", "views/", "router/"],
                "files": ["package.json", "vue.config.js", "src/main.js", "src/App.vue"],
        ...
        try:
            ...
            if primary_stack:
                result.update({
                    ...
                    "framework": primary_stack.name if primary_stack.type == "framework" else None,
                    "build_tool": primary_stack.name if primary_stack.type == "build_tool" else None,
                    "package_manager": self._extract_package_manager(combined_results),
                    "runtime_version": primary_stack.version,
                    "additional_technologies": [tech.name for tech in combined_results[1:5]],  # Top 5
                    "architecture_pattern": self._detect_architecture_pattern(context),
                    "deployment_strategy": self._suggest_deployment_strategy(combined_results)
                })
        ...
        try:
            ...
            return StackDetectionResult(
                ...
                analysis_metadata={
                    ...
                    "confidence_distribution": self._get_confidence_distribution(combined_results)
                }
            )
    ...
    async def _analyze_file_structure(self, context: AnalysisContext) -> List[TechnologyStack]:
        ...
        for framework, pattern in self.framework_patterns.items():
            ...
            if matches > 0:
                ...
                if final_confidence > 0.3:  # Minimum threshold
                    ...
                    results.append(TechnologyStack(
                        name=framework,
                        type="framework",
                        language=language,
                        version="unknown",
    ...
    def _analyze_file_extensions(self, context: AnalysisContext) -> List[TechnologyStack]:
    ...
    async def _analyze_package_files(self, context: AnalysisContext) -> List[TechnologyStack]:
    ...
    async def _analyze_file_content(self, context: AnalysisContext) -> List[TechnologyStack]:
    ...
    def _merge_technology_detections(self, detections: List[TechnologyStack]) -> TechnologyStack:
        ...
        return TechnologyStack(
            ...
            confidence=min(final_confidence + 0.1, 1.0),  # Boost for multiple confirmations
    ...
    async def _enhance_with_llm(
        self, 
        context: AnalysisContext, 
        current_results: List[TechnologyStack]
    ) -> List[TechnologyStack]:
    ...
    def _determine_primary_stack(self, results: List[TechnologyStack]) -> TechnologyStack:
    ...
    def _get_confidence_distribution(self, results: List[TechnologyStack]) -> Dict[str, int]:
    ...
    def _generate_suggestions(self, results: List[TechnologyStack]) -> List[str]:
        ...
        if not has_build_tool:
            suggestions.append("Consider adding a build tool for better dependency management")
        
        # Modern alternatives
        for result in results:
            if result.name == "jquery":
                suggestions.append("Consider migrating from jQuery to modern JavaScript frameworks")
    ...
    def _detect_architecture_pattern(self, context: AnalysisContext) -> Optional[str]:
        ...
        if any("service" in d.lower() for d in directories) and \
           any("repository" in d.lower() for d in directories):
            return "layered"
    ...
    def _suggest_deployment_strategy(self, results: List[TechnologyStack]) -> Optional[str]:
    ...
    async def get_supported_databases(self) -> List[str]:
        ...
        return [
            ...
            "mariadb", "elasticsearch", "cassandra", "dynamodb"
        ]
    ...
    async def get_supported_build_tools(self) -> List[str]:
        ...
        return [
            ...
            "maven", "gradle", "npm", "yarn", "pip", "cargo"
        ]
