...
class CodeAnalyzer:
    ...
    async def _analyze_javascript_code(self, file_path: str, content: str) -> FileAnalysis:
        ...
        functions_count = len(re.findall(r'function\s+\w+|=>\s*{|const\s+\w+\s*=\s*\(', content))
    ...
    async def _analyze_typescript_code(self, file_path: str, content: str) -> FileAnalysis:
    ...
    async def _analyze_java_code(self, file_path: str, content: str) -> FileAnalysis:
        ...
        classes_count = len(re.findall(r'class\s+\w+', content))
        functions_count = len(re.findall(r'public\s+\w+.*?\(|private\s+\w+.*?\(', content))
    ...
    async def _analyze_php_code(self, file_path: str, content: str) -> FileAnalysis:
        ...
        include_matches = re.findall(r'(?:include|require)(?:_once)?\s*\(\s*[\'"]([^\'"]+)', content)
    ...
    async def _analyze_go_code(self, file_path: str, content: str) -> FileAnalysis:
        ...
        classes_count = len(re.findall(r'type\s+\w+\s+struct', content))  # Structs in Go
        quality_issues = []
    ...
    def _analyze_architecture(self, file_tree: List[Dict]) -> Dict[str, Any]:
        ...
        for pattern_name, required_dirs in self.architecture_patterns.items():
            ...
            if matches >= len(required_dirs) * 0.6:  # 60% match threshold
                detected_patterns.append({
                    ...
                    "matched_components": [req_dir for req_dir in required_dirs 
                                         if any(req_dir in dir_path for dir_path in directories)]
                })
        ...
        return {
            ...
            "project_size": "small" if len(file_tree) < 50 else "medium" if len(file_tree) < 200 else "large"
        }
    ...
    def _calculate_code_metrics(
        ...
        high_severity_issues = sum(
            len([issue for issue in analysis.quality_issues if issue.severity == "high"])
            for analysis in file_analyses
        )
        
        medium_severity_issues = sum(
            len([issue for issue in analysis.quality_issues if issue.severity == "medium"])
            for analysis in file_analyses
        )
        
        # Calculate maintainability index (simplified)
        maintainability = max(0, 100 - avg_complexity - (high_severity_issues * 5) - (medium_severity_issues * 2))
        
        # Calculate technical debt ratio
        total_issues = high_severity_issues + medium_severity_issues
        technical_debt = min(total_issues / total_lines * 100, 100) if total_lines > 0 else 0
    ...
    def _summarize_patterns(self, framework_usage: Dict[str, int]) -> Dict[str, Any]:
        ...
        sorted_patterns = sorted(framework_usage.items(), key=lambda x: x[1], reverse=True)
