...
export const generate2FASecret = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await api.get("/api/v1/user/2fa/generate");
...
export const verify2FALogin = createAsyncThunk(
  ...
  async ({ token, userId }, { rejectWithValue }) => {
    try {
      const response = await api.post("/api/v1/user/2fa/verify", {
        token,
        userId,
      });
      return response.data.data;
...
export const disable2FA = createAsyncThunk(
  ...
  async (password, { rejectWithValue }) => {
    try {
      const response = await api.post("/api/v1/user/2fa/disable", { password });
...
export const generateNewBackupCodes = createAsyncThunk(
  ...
  async (password, { rejectWithValue }) => {
    try {
      const response = await api.post("/api/v1/user/2fa/backup-codes", {
        password,
      });
      return response.data.data;
    } catch (error) {
