...
const StickyFeaturesSection = ({
  ...
  return (
    <section
      ref={sectionRef}
      className="py-12 md:py-20 px-4 md:px-6 relative min-h-screen"
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="block lg:grid lg:grid-cols-2 gap-8 lg:gap-12">
          {/* Left Sidebar - Sticky Navigation (hidden on mobile, inline on mobile) */}
          <div className="hidden lg:block lg:sticky lg:top-24 lg:self-start">
            <div className="bg-gray-900/80 backdrop-blur-lg rounded-2xl p-4 md:p-6">
            ...
            {features.map((feature, index) => (
              ...
              >
                <motion.div
                  ...
                  className={`w-full bg-gradient-to-br from-gray-800/60 to-gray-900/60 backdrop-blur-lg border rounded-2xl p-4 md:p-8 transition-all duration-500 ${
                    activeFeature === index
                      ? "border-white/40 shadow-2xl shadow-white/10 scale-[1.01] lg:scale-[1.02]"
                      : "border-gray-700/50"
                  }`}
                >
                  {/* Feature Header */}
                  <div className="mb-6 md:mb-8">
                    <div
                      className={`p-3 md:p-4 bg-gradient-to-r ${gradient.replace(
                        "to-purple-500",
                        "/20 to-purple-500/20"
                      )} rounded-xl inline-block mb-4 md:mb-6`}
                    >
                      {React.createElement(feature.icon, {
                      ...
                      <div className="flex flex-wrap gap-2 md:gap-3">
                        {(feature.platforms || feature.techStacks || []).map(
                          (item, idx) => (
                            <span
                              key={idx}
                              className="px-3 md:px-4 py-1.5 md:py-2 bg-gray-700/50 border border-gray-600 rounded-full text-xs md:text-sm text-gray-300 hover:border-white/50 transition-colors"
                            >
                              {item}
                            </span>
                          )
