...
class UnifiedGenerator:
    ...
    async def generate_configurations(
        ...
        try:
            ...
            logger.info(f"Generating {len(config_types)} configuration types: {config_types}")
            ...
            logger.info(f"Configuration generation completed for {repo_name} in {generation_time:.2f}s")
            return enhanced_configs
            
        except Exception as e:
            generation_time = time.time() - start_time
            logger.error(f"Error generating configurations for {repo_name} after {generation_time:.2f}s: {e}", exc_info=True)
            return {
                "error": str(e),
                "metadata": {
                    "generated_at": time.time(),
    ...
    async def _generate_base_configurations(
        ...
        if "dockerfile" in config_types:
            ...
            try:
                ...
                logger.info(f"Dockerfile generated successfully in {dockerfile_time:.2f}s")
            except Exception as e:
                dockerfile_time = time.time() - dockerfile_start
                logger.error(f"Error generating Dockerfile after {dockerfile_time:.2f}s: {e}")
        ...
        if "docker_compose" in config_types:
            ...
            try:
                ...
                logger.info(f"Docker Compose generated successfully in {compose_time:.2f}s")
            except Exception as e:
                compose_time = time.time() - compose_start
                logger.error(f"Error generating Docker Compose after {compose_time:.2f}s: {e}")
        ...
        if "github_actions" in config_types:
            ...
            try:
                ...
                logger.info(f"GitHub Actions workflow generated successfully in {actions_time:.2f}s")
            except Exception as e:
                actions_time = time.time() - actions_start
                logger.error(f"Error generating GitHub Actions after {actions_time:.2f}s: {e}")
        ...
        if "kubernetes" in config_types:
            ...
            try:
                ...
                logger.info(f"Kubernetes manifests generated successfully in {k8s_time:.2f}s")
            except Exception as e:
                k8s_time = time.time() - k8s_start
                logger.error(f"Error generating Kubernetes manifests after {k8s_time:.2f}s: {e}")
        ...
        successful_configs = len([c for c in base_configs.values() if not c.get("error")])
        logger.info(f"Base configuration generation completed in {base_generation_time:.2f}s - {successful_configs}/{len(config_types)} successful")
    ...
    async def _enhance_configurations(
        ...
        try:
            ...
            if "error" in enhanced_configs:
                logger.warning(f"LLM enhancement failed after {enhancement_time:.2f}s: {enhanced_configs['error']}")
                return base_configs
            ...
            if merged_count > 0:
                logger.info(f"Merged {merged_count} fallback configurations from base configs")
            
            logger.info(f"LLM configuration enhancement completed successfully in {enhancement_time:.2f}s")
            return enhanced_configs
            
        except Exception as e:
            enhancement_time = time.time() - enhancement_start
            logger.error(f"Error during LLM enhancement after {enhancement_time:.2f}s: {e}", exc_info=True)
            # Fall back to base configurations
            logger.info("Falling back to base configurations due to LLM enhancement failure")
            return base_configs
