...
class BaseAnalyzer(ABC):
    ...
    def _extract_file_content(self, repository_data: Dict[str, Any], file_path: str) -> str:
    ...
    def _check_file_exists(self, repository_data: Dict[str, Any], file_path: str) -> bool:
        """Check if a file exists in the repository"""
        files = self._get_file_list(repository_data)
        return file_path in files or any(f.endswith(file_path) for f in files)
    
    def _find_files_by_pattern(self, repository_data: Dict[str, Any], pattern: str) -> list[str]:
    ...
    def _add_insight(self, insights: list[InsightModel], category: str, title: str, 
                    description: str, confidence: float, evidence: list[str] = None,
                    impact: str = "medium"):
