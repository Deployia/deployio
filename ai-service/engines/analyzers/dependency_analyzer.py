...
class DependencyAnalyzer:
    ...
    async def analyze_dependencies(
            ...
            if filename in self.analyzers:
                try:
                    ...
                    if result:
                        ...
                        vulnerability_count += result.get("potential_vulnerabilities", 0)
        ...
        return DependencyAnalysis(
            ...
            direct_dependencies=len([d for d in unique_dependencies if not d.transitive]),
    ...
    async def _analyze_package_json(self, content: str, file_path: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "package_manager": "npm",
                "potential_vulnerabilities": self._check_known_vulnerabilities(dependencies)
            }
    ...
    async def _analyze_pyproject_toml(self, content: str, file_path: str) -> Dict:
        ...
        dev_dep_pattern = r'\[tool\.poetry\.dev-dependencies\].*?(?=\[|\Z)'
        ...
        try:
            ...
            return {
                ...
                "dependencies": [
                    ...
                    for dep in analysis_result.dependencies[:50]  # Limit for response size
                ],
                ...
                "detected_files": [dep.file_source for dep in analysis_result.dependencies],
                ...
                "security_metrics": {
                    ...
                    "security_score": max(0, 100 - analysis_result.security_vulnerabilities * 10)
                }
            }
    ...
    async def get_supported_ecosystems(self) -> List[str]:
        ...
        return [
            "npm", "yarn", "pip", "pipenv", "poetry", "maven", "gradle",
            "composer", "cargo", "go", "bundler", "nuget"
        ]
    ...
    def _generate_recommendations(self, analysis_result: DependencyAnalysis) -> List[str]:
        ...
        version_quality = analysis_result.metrics.get("version_specification_quality", 0)
    ...
    def _generate_suggestions(self, analysis_result: DependencyAnalysis) -> List[str]:
        ...
        if dev_ratio > 0.7:
            suggestions.append("High ratio of dev dependencies - consider if all are necessary")
        elif dev_ratio < 0.1:
            suggestions.append("Consider adding development dependencies for testing and linting")
        ...
        if not categories.get("build_tools"):
            suggestions.append("Consider adding build tools for better development workflow")
    ...
    def _categorize_dependencies(self, dependencies: List[Dependency]) -> Dict[str, List[str]]:
        ...
        framework_packages = {
            ...
            "spring", "laravel", "rails", "sinatra"
        }
        
        database_packages = {
            "mysql", "postgresql", "mongodb", "redis", "sqlite",
            "psycopg2", "pymongo", "mysql-connector", "sequelize",
            "hibernate", "doctrine", "activerecord"
        }
        
        testing_packages = {
            "jest", "mocha", "chai", "cypress", "playwright", "selenium",
            "pytest", "unittest", "nose", "junit", "testng",
            "phpunit", "rspec", "minitest"
        }
        
        build_packages = {
            "webpack", "vite", "rollup", "parcel", "esbuild",
            "babel", "typescript", "sass", "less", "postcss"
        }
    ...
    def _check_known_vulnerabilities(self, dependencies: List[Dependency]) -> int:
        ...
        known_vulnerable = {
            "lodash", "moment", "jquery", "handlebars", "marked",
            "debug", "ms", "mime", "send", "serve-static"
        }
