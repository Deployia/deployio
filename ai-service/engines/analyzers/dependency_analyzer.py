...
class DependencyAnalyzer(BaseAnalyzer):
    ...
    def _init_dependency_patterns(self):
        ...
        self.dev_dependency_patterns = {
            "javascript": ["jest", "mocha", "chai", "eslint", "prettier", "webpack", "babel"],
            "python": ["pytest", "black", "flake8", "mypy", "sphinx"],
            "java": ["junit", "mockito", "checkstyle"],
        }
    
    async def analyze(self, repository_data: Dict[str, Any]) -> DependencyAnalyzerResult:
        ...
        try:
            ...
            for file_name, parser in self.package_parsers.items():
                if self._check_file_exists(repository_data, file_name):
                    file_content = self._extract_file_content(repository_data, file_name)
                    if file_content:
            ...
            confidence = self._calculate_dependency_confidence(analysis, package_managers)
            
            result.dependency_analysis = analysis
            result.insights = insights
            result.confidence = confidence
            
            logger.info(f"Dependency analysis completed: {analysis.total_dependencies} dependencies found")
        ...
        for line in lines:
            ...
            if line.startswith('['):
                ...
                if len(parts) == 2:
                    ...
                    dep_type = "production" if current_section == "packages" else "development"
        ...
        for pattern in dependency_patterns:
            ...
            for match in matches:
                ...
                if len(parts) >= 2:
                    ...
                    dep_type = "development" if "test" in pattern.lower() else "production"
    ...
    def _analyze_dependencies(self, dependencies: List[DependencyInfo]) -> DependencyAnalysis:
        ...
        analysis.production_dependencies = sum(1 for dep in dependencies if dep.type == "production")
        analysis.development_dependencies = sum(1 for dep in dependencies if dep.type == "development")
        ...
        # Health scoring
        analysis.health_score = self._calculate_health_score(dependencies, vulnerable_deps)
        
        # Generate recommendations
        analysis.security_recommendations = self._generate_security_recommendations(vulnerable_deps)
        analysis.optimization_suggestions = self._generate_optimization_suggestions(dependencies)
        
        analysis.dependencies = dependencies
        
        return analysis
    
    def _check_for_vulnerabilities(self, dependencies: List[DependencyInfo]) -> List[DependencyInfo]:
    ...
    def _calculate_health_score(self, dependencies: List[DependencyInfo], vulnerable_deps: List[DependencyInfo]) -> float:
    ...
    def _generate_security_recommendations(self, vulnerable_deps: List[DependencyInfo]) -> List[str]:
        """Generate security recommendations"""
        
        recommendations = []
        
        if vulnerable_deps:
            critical_count = sum(1 for dep in vulnerable_deps if dep.vulnerability_level == "critical")
            high_count = sum(1 for dep in vulnerable_deps if dep.vulnerability_level == "high")
            
            if critical_count > 0:
                recommendations.append(f"URGENT: Update {critical_count} critical vulnerability(ies)")
            
            if high_count > 0:
                recommendations.append(f"Update {high_count} high severity vulnerability(ies)")
    ...
    def _generate_optimization_suggestions(self, dependencies: List[DependencyInfo]) -> List[str]:
        ...
        if len(dependencies) > 100:
            suggestions.append("Consider reducing number of dependencies to improve build times")
        
        # Check for development dependencies in production
        dev_deps = [dep for dep in dependencies if dep.type == "development"]
        if len(dev_deps) > len(dependencies) * 0.5:
            suggestions.append("Large number of dev dependencies - ensure they're not included in production builds")
    ...
    def _calculate_dependency_confidence(self, analysis: DependencyAnalysis, package_managers: List[str]) -> float:
    ...
    def _generate_dependency_insights(self, analysis: DependencyAnalysis, dependencies: List[DependencyInfo]) -> List[InsightModel]:
        ...
        if analysis.vulnerable_count > 0:
            severity = "critical" if analysis.critical_vulnerabilities > 0 else "high" if analysis.high_vulnerabilities > 0 else "medium"
            self._add_insight(
                insights,
                "security",
        ...
        # Health insights
        health_status = "excellent" if analysis.health_score > 0.8 else "good" if analysis.health_score > 0.6 else "needs attention"
        self._add_insight(
            insights,
            "health",
            ...
            [f"Health score based on security and optimization factors"],
            "medium"
        )
