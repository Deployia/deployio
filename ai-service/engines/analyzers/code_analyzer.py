...
class CodeAnalyzer(BaseAnalyzer):
    ...
    def _init_analysis_patterns(self):
        ...
        self.architecture_patterns = {
            ...
            "layered": {
                "indicators": ["layers/", "presentation/", "business/", "data/", "dal/"],
                "confidence_boost": 0.2
            },
            "component_based": {
        ...
        self.design_patterns = {
            ...
            "observer": {
                "patterns": [r"addEventListener", r"observer", r"Observable", r"subscribe"],
                "language": ["python", "java", "javascript"]
            },
            "decorator": {
        ...
        try:
            ...
            for file_path in code_files:
                ...
                if file_content:
                    ...
                    # Complexity analysis
                    complexity = self._calculate_file_complexity(file_content, file_path)
            ...
            quality_score = self._calculate_quality_score(avg_complexity, code_smells, total_lines)
            maintainability_score = self._calculate_maintainability_score(quality_score, architecture_patterns)
            ...
            logger.info(f"Code analysis completed: {len(code_files)} files, quality score: {quality_score:.2f}")
    ...
    def _detect_code_smells(self, content: str, file_path: str) -> List[Dict[str, Any]]:
        ...
        for smell_name, smell_config in self.code_smell_patterns.items():
            ...
            if smell_name == "long_function":
                ...
                function_matches = re.finditer(pattern, content, re.DOTALL | re.MULTILINE)
    ...
    def _detect_architecture_patterns(self, repository_data: Dict[str, Any]) -> List[str]:
    ...
    def _detect_design_patterns(self, repository_data: Dict[str, Any], code_files: List[str]) -> List[str]:
    ...
    def _calculate_quality_score(self, complexity: float, code_smells: List[Dict], total_lines: int) -> float:
    ...
    def _calculate_maintainability_score(self, quality_score: float, architecture_patterns: List[str]) -> float:
    ...
    def _calculate_code_confidence(self, analysis: CodeAnalysis, code_files: List[str]) -> float:
    ...
    def _generate_code_insights(self, analysis: CodeAnalysis, code_files: List[str]) -> List[InsightModel]:
        ...
        # Quality insight
        quality_level = "excellent" if analysis.quality_score > 0.8 else "good" if analysis.quality_score > 0.6 else "needs improvement"
        quality_impact = "low" if analysis.quality_score > 0.7 else "medium" if analysis.quality_score > 0.5 else "high"
        ...
        self._add_insight(
            ...
            [f"Based on complexity and code smell analysis"],
            quality_impact
        )
        
        # Complexity insight
        if analysis.complexity_score > 0:
            complexity_level = "high" if analysis.complexity_score > 5 else "moderate" if analysis.complexity_score > 2 else "low"
            self._add_insight(
                insights,
                "complexity",
                ...
                0.8,
                [f"Complexity measured by control flow and nesting"],
        ...
        if analysis.code_smells:
            ...
            high_severity_smells = sum(1 for smell in analysis.code_smells if smell.get("severity") == "high")
            ...
            self._add_insight(
                ...
                0.8,
                [f"{high_severity_smells} high severity issues" if high_severity_smells else "No high severity issues"],
                impact
            )
