...
class AnalyzerEnhancer:
    ...
    async def enhance_technology_stack(
        ...
        if not self.is_available:
            logger.warning("No LLM providers available for technology stack enhancement")
    ...
    async def enhance_dependency_analysis(
        ...
        """
        if not self.is_available:
            logger.warning("No LLM providers available for dependency analysis enhancement")
            return analysis_result
    ...
    async def generate_comprehensive_insights(
        ...
        try:
            ...
            request = LLMRequest(
                system_prompt=prompt_data["system"],
                user_prompt=prompt_data["user"],
                max_tokens=self.max_tokens * 2,  # More tokens for comprehensive insights
                temperature=self.temperature
            )
        ...
        try:
            ...
            if 'vulnerabilities' in enhancements:
                ...
                if isinstance(vuln_info, dict) and 'summary' in vuln_info:
                    analysis_result.insights.append(f"Vulnerabilities: {vuln_info['summary']}")
        ...
        try:
            ...
            if 'maintainability_score' in enhancements:
                ...
                if isinstance(score, (int, float)) and 0 <= score <= 100:
                    ...
                    analysis_result.insights.append(f"Maintainability Score: {score}/100")
        ...
        try:
            ...
            return {
                "analyzer_enhancer": {
                    ...
                    "prompts_loaded": hasattr(self.prompts, "technology_stack_enhancement"),
                }
            }
        except Exception as e:
