...
class ConfigurationGenerator:
            ...
            if config_type in self.generators:
                try:
                    config = await self.generators[config_type](
                        analysis_result.technology_stack, deployment_target
                    )
                    if config:
    ...
    async def _generate_docker_compose_config(
        self, stack: TechnologyStack, deployment_target: str
    ) -> GeneratedConfig:
        ...
        app_service = {
            "build": ".",
            "ports": [
                f"{self._get_default_port(stack)}:{self._get_default_port(stack)}"
            ],
    ...
    async def _generate_kubernetes_config(
        ...
        app_name = (
            stack.project_name.lower().replace(" ", "-")
            if stack.project_name
            else "app"
        )
                        ...
                        "containers": [
                            {
                                ...
                                "ports": [
                                    {"containerPort": self._get_default_port(stack)}
                                ],
                                "env": self._get_k8s_environment(
                                    stack, deployment_target
                                ),
        ...
        service = {
            ...
            "spec": {
                ...
                "type": (
                    "LoadBalancer" if deployment_target == "production" else "NodePort"
                ),
            },
        }
                ...
                env_vars.update(
                    {
                        ...
                        f"{db_name}_PORT": str(
                            self._get_database_port(db.get("name", "postgres"))
                        ),
    ...
    def _generate_resource_recommendations(
        ...
        return {"cpu": "500m", "memory": "512Mi", "storage": "10Gi", "replicas": 3}
    ...
    def _get_database_port(self, db_name: str) -> int:
        ...
        ports = {"postgres": 5432, "mysql": 3306, "mongodb": 27017, "redis": 6379}

        return ports.get(db_name.lower(), 5432)

    def _generate_database_service(
        self, db: Dict[str, Any], target: str
    ) -> Dict[str, Any]:
    ...
    def _get_compose_environment(
        self, stack: TechnologyStack, target: str
    ) -> List[str]:
        """Get environment variables for Docker Compose"""

        env_vars = [f"NODE_ENV={target}", f"PORT={self._get_default_port(stack)}"]

        return env_vars

    def _get_k8s_environment(
        self, stack: TechnologyStack, target: str
    ) -> List[Dict[str, str]]:
