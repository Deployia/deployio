...
class CacheManager:
    ...
    def __init__(self):
        ...
        self.enabled = REDIS_AVAILABLE  # Changed: enable if Redis is available

        if self.enabled:
            self._initialize_redis()
        else:
            logger.warning(
                "Cache manager disabled - Redis not available or not configured"
            )
    ...
    def _generate_cache_key(
        self, repository_url: str, analysis_type: str = "full"
    ) -> str:
    ...
    async def store_analysis_result(
        ...
        try:
            ...
            logger.info(
                f"Cached analysis result for {repository_url} (TTL: {cache_ttl}s)"
            )
            return True
    ...
    async def invalidate_repository_cache(self, repository_url: str) -> int:
        ...
        try:
            ...
            pattern = (
                f"analysis:*{hashlib.md5(repository_url.encode()).hexdigest()[:8]}*"
            )
    ...
    async def get_repository_metadata(
        ...
        try:
            metadata_key = (
                f"metadata:{hashlib.md5(repository_url.encode()).hexdigest()}"
            )
    ...
    async def store_repository_metadata(
        self, repository_url: str, metadata: Dict[str, Any], ttl: Optional[int] = None
    ) -> bool:
        """
        ...
        try:
            ...
            await self.redis_client.setex(
                metadata_key, cache_ttl, json.dumps(metadata, default=str)
            )
