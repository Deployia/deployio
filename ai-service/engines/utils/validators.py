...
class InputValidator:
    ...
    def validate_repository_url(self, url: str) -> ValidationResult:
        ...
        is_supported = (
            self.github_url_pattern.match(url)
            or self.gitlab_url_pattern.match(url)
            or self.bitbucket_url_pattern.match(url)
        )

        if not is_supported:
            errors.append(
                "Unsupported repository platform. Supported: GitHub, GitLab, Bitbucket"
            )
            return ValidationResult(False, errors, warnings)
        ...
        for key, value in options.items():
            if key == "include_dependencies":
                ...
                if not isinstance(value, str) or value not in [
                    ...
                    errors.append(
                        "analysis_depth must be 'shallow', 'medium', or 'deep'"
                    )
                else:
                    sanitized_options[key] = value
                ...
                if isinstance(value, list):
                    ...
                    if len(valid_languages) != len(value):
                        ...
                        warnings.append(
                            f"Unsupported languages ignored: {', '.join(invalid_langs)}"
                        )
    ...
    def validate_code_content(
        self, content: str, max_size: int = 1048576
    ) -> ValidationResult:
    ...
    def validate_configuration_request(
        ...
        if "config_type" in request_data:
            ...
            if request_data["config_type"] not in valid_config_types:
                errors.append(
                    f"Invalid config_type. Valid types: {', '.join(valid_config_types)}"
                )

        # Validate technology_stack
        if "technology_stack" in request_data:
            stack_result = self.validate_technology_stack(
                request_data["technology_stack"]
            )
            if not stack_result.is_valid:
                errors.extend(
                    [
                        f"Technology stack error: {error}"
        ...
        if "options" in request_data:
            ...
            if not options_result.is_valid:
                errors.extend(
                    [f"Options error: {error}" for error in options_result.errors]
                )
            else:
    ...
    def validate_technology_stack(self, stack_data: Dict[str, Any]) -> ValidationResult:
        ...
        if "frameworks" in stack_data:
            if isinstance(stack_data["frameworks"], list):
                valid_frameworks = [
                    fw
                    for fw in stack_data["frameworks"]
                    if fw.lower() in self.supported_frameworks
                ]
                sanitized_stack["frameworks"] = valid_frameworks

                invalid_frameworks = set(stack_data["frameworks"]) - set(
                    valid_frameworks
                )
                if invalid_frameworks:
                    warnings.append(
                        f"Unsupported frameworks ignored: {', '.join(invalid_frameworks)}"
                    )
            else:
    ...
    def validate_batch_request(
        self, repositories: List[str], options: Optional[Dict[str, Any]] = None
    ) -> ValidationResult:
        ...
        for i, repo_url in enumerate(repositories):
            ...
            if repo_result.is_valid:
                ...
                errors.extend(
                    [f"Repository {i+1}: {error}" for error in repo_result.errors]
                )
