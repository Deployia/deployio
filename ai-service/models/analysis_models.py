...
class BuildConfiguration:
    ...
    # Build information
    build_commands: Dict[str, str] = field(
        default_factory=dict
    )  # {"build": "npm run build"}
    install_commands: Dict[str, str] = field(
        default_factory=dict
    )  # {"install": "npm install"}
    start_commands: Dict[str, str] = field(
        default_factory=dict
    )  # {"start": "npm start"}
    ...
    )  # ["src/index.js", "main.py"]
    config_files: List[str] = field(
        default_factory=list
    )  # ["package.json", "requirements.txt"]
    dockerfile_hints: List[str] = field(
        default_factory=list
...
class AnalysisResult:
    ...
    def has_vulnerabilities(self) -> bool:
        ...
        return (
            self.dependency_analysis and self.dependency_analysis.vulnerable_count > 0
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result_dict["confidence_level"] = (
            self.confidence_level.value if self.confidence_level else "low"
        )
        result_dict["analysis_types"] = [t.value for t in self.analysis_types]

        # Ensure all nested dictionaries are properly serializable
        return result_dict
