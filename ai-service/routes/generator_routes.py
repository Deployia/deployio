...
class GenerateFromAnalysisRequest(BaseModel):
    ...
    user_preferences: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="User preferences for generation"
    )
...
class GenerateConfigRequest(BaseModel):
    ...
    config_type: str = Field(
        ..., description="Configuration type: docker_compose, kubernetes"
    )
    optimization_level: str = Field(
        default="balanced", description="Optimization level"
    )
...
def create_generator_routes() -> APIRouter:
    ...
    async def generate_complete_configuration(request: GenerateFromAnalysisRequest):
        ...
        try:
            logger.info(
                f"Generating complete configuration for {len(request.config_types)} types"
            )
            ...
            pipeline_types = [
                t
                for t in request.config_types
                if t in ["github_actions", "gitlab_ci", "jenkins"]
            ]
            ...
            successful_generations = sum(
                1 for config in generated_configs.values() if "error" not in config
            )
    ...
    async def get_supported_types():
        ...
        """
        return {
            "config_types": ["dockerfile", "docker_compose", "kubernetes"],
            "pipeline_platforms": ["github_actions", "gitlab_ci", "jenkins"],
            "optimization_levels": ["basic", "balanced", "performance", "security"],
            "features": {
